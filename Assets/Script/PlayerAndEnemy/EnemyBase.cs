...
public class EnemyBase : Entity,IPunObservable
{
    ...
    [SerializeField] protected float attackCooldownTimer=1f;
    ...
    [PunRPC]
    protected virtual void FlipRPC(bool faceRight)
    {
    ...
    protected virtual void FindLocalPlayer()
    {
    ...
    protected virtual void AnimatorController() //플레이어 애니메이션 관리 
    {
    ...
    public void HitedRPC(float _damageDone, Vector2 _hitDirection)
    {
     ...
     //코루틴 예시
    protected virtual IEnumerator StopTakeDamageSkeleton()
    {
        yield return new WaitForSeconds(1f);
        isTakeDamage = false;
    }

    protected virtual void EnemyDyingCheck()
    {
        if (Hp <= 0 && !isEnemyDie)
        {
            isTakeDamage= false;
    ...
    protected virtual void RespawnEnemy(Vector3 respawnPosition)
    {
        EnemyManager enemyManager = FindObjectOfType<EnemyManager>();
        if (enemyManager != null && PhotonNetwork.IsMasterClient)
        {
    ...
    [PunRPC]
    protected virtual void RequestDestroy(int viewID, Vector3 respawnPosition)
    {
